 ...
 * Run: pnpm --filter @workspace/scripts exec tsx src/migrate-plans.ts
 */
import pg from "pg";
...
async function run() {
  ...
  try {
    ...
    `);
    const pfResult = await client.query(`SELECT COUNT(*) FROM plan_features`);
    console.log(`  ✓ plan_features: ${pfResult.rows[0].count} rows inserted\n`);
    ...
    `);
    const spResult = await client.query(`SELECT COUNT(*) FROM subscription_plans`);
    console.log(`  ✓ subscription_plans: ${spResult.rows[0].count} rows inserted\n`);
    ...
    `);
    await client.query(`
      INSERT INTO b2b_plan_config
        (plan_id, min_seats, max_seats, crm_included, crm_monthly_charge, base_features)
      VALUES
    ...
    `);
    const b2bResult = await client.query(`SELECT COUNT(*) FROM b2b_plan_config`);
    console.log(`  ✓ b2b_plan_config: ${b2bResult.rows[0].count} rows\n`);
    ...
    const pf = await client.query(
      `SELECT feature_name, free_value, max_value, pro_value, family_value
         FROM plan_features ORDER BY feature_name`
    );
    console.log("📋 plan_features:");
    pf.rows.forEach(r =>
      console.log(
        `  ${r.feature_name.padEnd(30)} FREE=${String(r.free_value).padEnd(12)} MAX=${String(r.max_value).padEnd(12)} PRO=${String(r.pro_value).padEnd(15)} FAMILY=${r.family_value}`
      )
    );

    const sp = await client.query(
      `SELECT plan_id, plan_name, plan_type, price_monthly, price_yearly
         FROM subscription_plans ORDER BY display_order`
    );
    console.log("\n📋 subscription_plans:");
    sp.rows.forEach(r =>
      console.log(
        `  ${r.plan_id.padEnd(15)} ${r.plan_name.padEnd(10)} ${r.plan_type.padEnd(12)} ₹${r.price_monthly}/mo  ₹${r.price_yearly}/yr`
      )
    );

    const b2b = await client.query(
      `SELECT plan_id, min_seats, max_seats, crm_included, base_features FROM b2b_plan_config`
    );
    console.log("\n📋 b2b_plan_config:");
    b2b.rows.forEach(r =>
      console.log(
        `  ${r.plan_id.padEnd(15)} seats:${r.min_seats}-${r.max_seats}  crm:${r.crm_included}  base:${r.base_features}`
      )
    );
