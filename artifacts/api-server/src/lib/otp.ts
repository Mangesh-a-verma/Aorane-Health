...
/**
 * Email OTPs are stored under an "email:"-prefixed key (both in the
 * otp_store DB table's `phone` column, which doubles as a generic
 * identifier column, and in the in-memory cache) so they don't collide
 * with phone-number OTPs, which are stored under the raw phone number.
 * ALWAYS use this helper when storing or looking up an email OTP —
 * reconstructing the prefix manually in multiple places is exactly how
 * the delete-account flow ended up looking up the wrong key and
 * silently failing every email-OTP verification (fixed July 2026).
 */
...
export async function sendEmailOtp(email: string, otp: string): Promise<boolean> {
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || "noreply@aorane.com";
  if (!apiKey) {
