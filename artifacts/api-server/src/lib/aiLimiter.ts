...
async function fetchFeatureRow(feature: AIFeature): Promise<CachedFeature | null> {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT free_value, max_value, pro_value, family_value
...
/** Get today's usage count for a user+feature */
async function getTodayUsage(userId: string, feature: AIFeature): Promise<number> {
  try {
    const { rows } = await pool.query(
      `SELECT usage_count FROM ai_usage_daily
         WHERE user_id = $1 AND feature_name = $2 AND usage_date = $3`,
      [userId, feature, todayIST()],
    );
...
export async function checkAndUseAILimit(
  ...
  if (limit === 0) {
    ...
    return { allowed: false, remaining: 0, limit: 0, usedToday: 0, planRequired };
  }
  ...
  const usedToday = await getTodayUsage(userId, feature);

  // Over daily limit
  if (usedToday >= limit) {
    return { allowed: false, remaining: 0, limit, usedToday };
  }

  // Under limit — increment and allow
  await incrementAIUsage(userId, feature);
  return { allowed: true, remaining: limit - usedToday - 1, limit, usedToday };
}
