...
// FIX C3 + B1 — Issue fresh tokens after plan change so the client immediately
// gets a JWT with the updated `plan` claim. Without this, the user keeps the old
// JWT (valid 30 days) which the server reads via req.userPlan, causing every
// AI feature (food scan, smart-scan, diet plan, etc.) to fail with
// "Upgrade to MAX" even though the user just upgraded.
async function rotateUserTokensForPlan(userId: string): Promise<{ accessToken: string; refreshToken: string; plan: string }> {
  ...
  // Invalidate cached plan so the very next request reads the fresh DB value
...
router.post("/payment/verify", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // FIX C3 + B1 — issue fresh JWT with new plan claim
    ...
    res.json({
      ...
      expiresAt,
      inviteCode,
      // FIX C3 — Client MUST replace its stored tokens with these to immediately
      // see the new plan in subsequent requests. Mobile/web should persist these
      // to AsyncStorage / localStorage right after success.
...
router.post("/payment/subscription/create", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (!isLiveMode()) {
      ...
      await db.update(usersTable).set({ plan: plan as "free" | "pro" | "max" | "family" }).where(eq(usersTable.id, req.userId!));
      // FIX C3 + B1 — issue fresh JWT with new plan claim (test mode)
      const tokens = await rotateUserTokensForPlan(req.userId!).catch(() => null);
      return res.json({
