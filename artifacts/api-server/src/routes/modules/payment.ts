...
// ─── GET: Self-test payment security (temporary, admin-secret protected) ──────
router.get("/payment/self-test", async (req, res) => {
  if (req.headers["x-admin-secret"] !== "aorane_selftest_2026") {
  ...
  try {
    ...
    const [payment] = await db.insert(paymentsTable).values({
      ...
      plan: "pro", seats: 1, razorpayOrderId: testOrderId, status: "pending",
    }).returning();

    // 2. Generate a valid JWT for this test user
    const testToken = signUserToken({ userId: testUserId, plan: "free", phone: "0000000000" });

    // 3. SECURITY TEST: Try isTestMode bypass (should FAIL in live mode)
    const bypassResp = await fetch(`http://localhost:${process.env["PORT"] || 8080}/api/payment/verify`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${testToken}` },
      body: JSON.stringify({ paymentId: payment.id, plan: "pro", isTestMode: true }),
    });
    const bypassBody = await bypassResp.json() as Record<string, unknown>;
    results["security_isTestMode_bypass"] = {
      status: bypassResp.status,
      response: bypassBody,
      bypassed: bypassResp.ok && (bypassBody.success === true),
      expected: "BLOCKED (403/400)",
      passed: !bypassResp.ok,
    };

    // 4. SECURITY TEST: Verify with wrong signature (should FAIL)
    const wrongSigResp = await fetch(`http://localhost:${process.env["PORT"] || 8080}/api/payment/verify`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${testToken}` },
      body: JSON.stringify({ paymentId: payment.id, razorpayOrderId: testOrderId, razorpayPaymentId: "pay_fake123", razorpaySignature: "invalidsignature", plan: "pro" }),
    });
    const wrongSigBody = await wrongSigResp.json() as Record<string, unknown>;
    results["security_invalid_signature"] = {
      status: wrongSigResp.status,
      response: wrongSigBody,
      passed: !wrongSigResp.ok,
    };

    // 5. TEST: order-status returns "pending" (not "success")
    const statusResp = await fetch(`http://localhost:${process.env["PORT"] || 8080}/api/payment/order-status?orderId=${testOrderId}`, {
      headers: { "Authorization": `Bearer ${testToken}` },
    });
    const statusBody = await statusResp.json() as Record<string, unknown>;
    results["order_status_pending"] = {
      status: statusResp.status,
      paymentStatus: statusBody.status,
      passed: statusBody.status === "pending",
    };

    // 6. TEST: rzp-callback with fake data (should return "Payment Cancelled" HTML)
    const callbackResp = await fetch(`http://localhost:${process.env["PORT"] || 8080}/api/payment/rzp-callback`, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: "razorpay_payment_id=pay_FAKE&razorpay_order_id=" + testOrderId + "&razorpay_signature=BADSIG",
    });
    const callbackHtml = await callbackResp.text();
    results["callback_invalid_signature"] = {
      status: callbackResp.status,
      showsCancelled: callbackHtml.includes("Payment Cancelled"),
      doesNotActivate: !callbackHtml.includes("Payment Successful"),
      passed: callbackHtml.includes("Payment Cancelled") && !callbackHtml.includes("Payment Successful"),
    };

    // 7. Cleanup test data
    await db.delete(paymentsTable).where(eq(paymentsTable.id, payment.id));

    const allPassed = Object.values(results).every((r) => (r as { passed: boolean }).passed);
    res.json({ allPassed, liveMode: isLiveMode(), results });
  } catch (e) {
