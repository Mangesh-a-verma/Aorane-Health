...
import { sendSupportReplyEmail } from "../../lib/welcome-email";
...
router.patch("/admin/support-tickets/:id", requireAdmin, async (req, res) => {
  try {
    ...
    // Notify the ticket's owner by email if the admin left a reply (notes)
    // or changed the status — but not on a bare priority-only change.
    if (admin_notes !== undefined || status) {
      try {
        const ticketRow = await pool.query(
          `SELECT user_email, user_name, subject, admin_notes, status FROM support_tickets WHERE id = $1`,
          [id]
        );
        const t = ticketRow.rows[0];
        if (t?.user_email && t.admin_notes) {
          sendSupportReplyEmail({
            toEmail: t.user_email,
            name: t.user_name || "",
            subject: t.subject,
        ...
        logger.warn({ err: emailErr }, "[support] Failed to send ticket-reply email (non-fatal)");
      }
    }
