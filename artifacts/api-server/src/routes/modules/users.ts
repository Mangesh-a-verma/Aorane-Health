...
router.patch("/users/profile", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (dateOfBirth       !== undefined) {
      // ─── AGE GATE: AORANE is 18+ only (no parental-consent flow exists
      // for processing a minor's health data under DPDPA 2023 §9), so
      // block account use entirely rather than accept the profile update.
      const age = calculateAge(String(dateOfBirth));
      if (age !== null && age < MINIMUM_AGE_YEARS) {
        await pool.query(`UPDATE users SET is_active=false WHERE id=$1`, [req.userId!]);
        await invalidateUserActiveCache(req.userId!);
        // Force-logout: invalidate any tokens already issued for this session
        // (same mechanism used by /auth/logout), so requireAuth rejects them
        // on the very next request even without a new deploy.
        await cache.set(`logout:user:${req.userId}`, Date.now().toString(), 60 * 60 * 24 * 400);
        res.status(403).json({
          error: "AGE_RESTRICTED: AORANE is only available to users who are 18 years or older. Your account has been deactivated.",
        });
        return;
      }
...
router.get("/users/scorecard", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const foodPct     = freshScore?.foodScore     != null ? Math.round(freshScore.foodScore)     : 0;
    const waterPct    = freshScore?.waterScore    != null ? Math.round(freshScore.waterScore)    : 0;
    const exercisePct = freshScore?.exerciseScore != null ? Math.round(freshScore.exerciseScore) : 0;
    const medicinePct = freshScore?.medicineScore != null ? Math.round(freshScore.medicineScore) : 0;
...
router.get("/users/search", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // CRIT-5: Enforce privacy settings — only return users who allow basic profile sharing
