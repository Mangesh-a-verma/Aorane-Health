...
import { Heart, Brain, Smartphone, Building2, ArrowRight, Play, TrendingUp, Users, Activity } from "lucide-react";
import {
...
function B2CDashboard() {
  return (
    <div className="space-y-3">
      ...
      <div className="bg-gradient-to-r from-[#0747A6] to-[#1565C0] rounded-2xl p-4 text-white">
        <p className="text-xs font-semibold opacity-70 mb-1">AI Health Score</p>
        <div className="flex items-center gap-3">
          <span className="text-4xl font-extrabold">87</span>
          <div>
            <span className="text-xs font-bold bg-white/20 px-2 py-0.5 rounded-full">Excellent</span>
            <p className="text-xs opacity-70 mt-1">+4 pts from last week</p>
          </div>
        </div>
      </div>
    </div>
...
export default function HeroSection({ audience, onSignUp }: HeroSectionProps) {
  const b2cContent = {
    ...
    cta1: { label: "Download Free App", href: "https://github.com/Mangesh-a-verma/Health-Data-Hub/releases/download/v2.0/Aorane.v2.apk", icon: Smartphone },
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
          ...
          >
            <motion.div
              ...
              className="inline-flex items-center gap-2 bg-[#0747A6]/10 text-[#0747A6] text-xs font-semibold px-3 py-1.5 rounded-full mb-6"
            >
              <span className="w-2 h-2 bg-[#10B981] rounded-full animate-pulse" />
              {c.badge}
            </motion.div>

            <h1 className="text-4xl sm:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] tracking-tight text-gray-900 mb-6">
              {c.headline.split("\n").map((line, i) => (
                <span key={i}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <motion.a
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex items-center justify-center gap-2 px-6 py-3.5 bg-white text-[#0747A6] rounded-2xl font-semibold text-sm shadow border border-[#0747A6]/15 hover:bg-[#0747A6]/5 transition-colors"
              >
            ...
            <div className="flex items-center gap-6 mt-8 pt-8 border-t border-gray-200">
              {[
                ...
                <div key={s.label}>
                  <p className="text-2xl font-extrabold text-[#0747A6]">{s.val}</p>
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="absolute -top-12 -right-12 w-72 h-72 bg-[#0747A6]/8 rounded-full blur-3xl" />
            <div className="absolute -bottom-8 -left-8 w-56 h-56 bg-[#10B981]/10 rounded-full blur-3xl" />

            <div className="relative float-card">
              <div className="bg-white/70 backdrop-blur rounded-3xl border border-white p-5 shadow-xl">
