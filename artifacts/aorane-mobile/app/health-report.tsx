...
type StressReport = { weekAvg: number; highStreakDays: number; burnoutRisk: boolean; totalLogs: number; days: Array<{ date: string; dayLabel: string; avgScore: number; count: number }> };
...
<div class="doc">
  ...
  ${stress && stress.totalLogs > 0 ? `
  <div class="ruler"></div>
  <div class="section">
    <div class="section-title">Stress Index — This Week</div>
    <div class="score-row" style="align-items:center;gap:16px">
      <div class="score-circle" style="border-color:${stressColor(stress.weekAvg || 0)};min-width:80px">
        <span class="score-num" style="color:${stressColor(stress.weekAvg || 0)}">${stress.weekAvg || 0}</span>
        <span class="score-lbl" style="color:${stressColor(stress.weekAvg || 0)}">${stressLabel(stress.weekAvg || 0)}</span>
      </div>
      <div style="flex:1">
        <div style="margin-bottom:6px;font-size:10px;color:#0D1F33"><strong>Weekly Avg Stress Score:</strong> ${stress.weekAvg || 0}/100</div>
        <div style="margin-bottom:6px;font-size:10px;color:#0D1F33"><strong>Check-ins Logged:</strong> ${stress.totalLogs} entries</div>
        <div style="margin-bottom:6px;font-size:10px;color:#0D1F33"><strong>Status:</strong> <span style="color:${stressColor(stress.weekAvg || 0)};font-weight:bold">${stressLabel(stress.weekAvg || 0)}</span></div>
        ${stress.burnoutRisk ? `<div style="background:#FEE2E2;border-radius:6px;padding:6px 10px;font-size:9px;color:#EF4444;font-weight:bold">⚠️ Burnout Risk Detected — 3+ consecutive high-stress days. Please consult a healthcare professional.</div>` : ""}
        ${stress.highStreakDays >= 2 ? `<div style="font-size:9px;color:#F97316;margin-top:4px">${stress.highStreakDays} elevated-stress days in a row</div>` : ""}
      </div>
    </div>
    <table style="margin-top:12px">
      <tr><th>Parameter</th><th>Value</th><th>Category</th></tr>
      <tr><td>Weekly Avg Stress Index</td><td><strong>${stress.weekAvg || 0}/100</strong></td><td style="color:${stressColor(stress.weekAvg||0)};font-weight:bold">${stressLabel(stress.weekAvg||0)}</td></tr>
      <tr><td>Check-ins Completed</td><td><strong>${stress.totalLogs}</strong></td><td style="color:#10B981;font-weight:bold">${stress.totalLogs > 0 ? "Active" : "None"}</td></tr>
      <tr><td>High-Stress Streak</td><td><strong>${stress.highStreakDays} days</strong></td><td style="color:${stress.highStreakDays >= 3 ? "#EF4444" : "#10B981"};font-weight:bold">${stress.highStreakDays >= 3 ? "Burnout Risk" : stress.highStreakDays >= 2 ? "Monitor" : "Normal"}</td></tr>
      <tr><td>Burnout Risk</td><td><strong>${stress.burnoutRisk ? "YES" : "NO"}</strong></td><td style="color:${stress.burnoutRisk ? "#EF4444" : "#10B981"};font-weight:bold">${stress.burnoutRisk ? "Consult Doctor" : "Safe"}</td></tr>
    </table>
    <div class="insight-box" style="background:#F5F0FF;margin-top:14px">
      <div class="insight-title" style="color:#7C3AED">🧠 STRESS INTERPRETATION</div>
      <div class="insight-text" style="color:#4C1D95">
        ${(stress.weekAvg||0) < 26 ? "Your stress level is low this week. Continue your current wellness routine." :
          (stress.weekAvg||0) < 51 ? "Moderate stress detected. Consider daily breathing exercises and adequate sleep." :
          (stress.weekAvg||0) < 76 ? "Elevated stress detected this week. Prioritize rest, reduce workload if possible, and practice relaxation techniques." :
          "High stress levels detected. This may impact physical health. Please consult your healthcare provider."}
        <br/><small style="opacity:0.7">Stress is scored 0–100. Below 26 is low, 26–50 moderate, 51–75 elevated, above 75 is high risk.</small>
      </div>
    </div>
  </div>` : ""}
...
export default function HealthReportScreen() {
  ...
  const loadData = async () => {
    try {
      const [sc, co, stressRes] = await Promise.all([api.getScorecard(), api.getCompanySettings(), api.getStressWeekly()]);
      setCard(sc as Scorecard);
      setCompany({ ...DEFAULT_CO, ...co.settings });
      setStressData(stressRes as StressReport);
    } catch { }
    setLoading(false);
  };
  ...
  const handleDownload = async () => {
    ...
    if (Platform.OS === "web") {
      ...
      const html = buildReportHtml(card, company, reportType, dateRange, generatedAt, stressData);
      ...
      <ScrollView contentContainerStyle={{ paddingTop: topPad + 12, paddingBottom: 100, paddingHorizontal: 12 }}>
        ...
        {loading ? (
          <View style={{ alignItems: "center", paddingTop: 80 }}>
            ...
            <Text style={{ color: "#7A90A4", fontFamily: "Inter_400Regular", marginTop: 12 }}>Preparing your report...</Text>
          </View>
        ) : (
          <>
            <View
            ...
            >
              ...
              {stressData && stressData.totalLogs > 0 && (
                <>
                  <View style={styles.ruler} />
                  <View style={styles.section}>
                    <Text style={[styles.sectionTitle, { borderColor: "#7C3AED" }]}>STRESS INDEX — THIS WEEK</Text>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 14 }}>
                      <View style={{ width: 80, height: 80, borderRadius: 40, borderWidth: 3, borderColor: stressColor(stressData.weekAvg || 0), alignItems: "center", justifyContent: "center" }}>
                        <Text style={{ fontFamily: "Inter_700Bold", fontSize: 22, color: stressColor(stressData.weekAvg || 0) }}>{stressData.weekAvg || 0}</Text>
                        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 8, color: stressColor(stressData.weekAvg || 0) }}>{stressLabel(stressData.weekAvg || 0)}</Text>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: "#0D1F33", marginBottom: 4 }}>
                          <Text style={{ fontFamily: "Inter_600SemiBold" }}>Weekly Avg: </Text>{stressData.weekAvg || 0}/100
                        </Text>
                        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: "#0D1F33", marginBottom: 4 }}>
                          <Text style={{ fontFamily: "Inter_600SemiBold" }}>Check-ins: </Text>{stressData.totalLogs} logged
                        </Text>
                        {stressData.burnoutRisk && (
                          <View style={{ backgroundColor: "#FEE2E2", borderRadius: 6, padding: 6 }}>
                            <Text style={{ color: "#EF4444", fontFamily: "Inter_700Bold", fontSize: 9 }}>⚠️ Burnout Risk Detected</Text>
                          </View>
                        )}
                      </View>
                    </View>
                    <View style={styles.table}>
                      <View style={[styles.tableRow, { backgroundColor: "#7C3AED18" }]}>
                        <Text style={[styles.thCell, { flex: 2 }]}>STRESS PARAMETER</Text>
                        <Text style={[styles.thCell, { flex: 1.5 }]}>VALUE</Text>
                        <Text style={[styles.thCell, { flex: 1 }]}>STATUS</Text>
                      </View>
                      {[
                        { param: "Weekly Avg Stress Index", value: `${stressData.weekAvg || 0}/100`, color: stressColor(stressData.weekAvg || 0), cat: stressLabel(stressData.weekAvg || 0) },
                        { param: "Check-ins Completed", value: `${stressData.totalLogs}`, color: "#10B981", cat: stressData.totalLogs > 0 ? "Active" : "None" },
                        { param: "High-Stress Streak", value: `${stressData.highStreakDays} days`, color: stressData.highStreakDays >= 3 ? "#EF4444" : "#10B981", cat: stressData.highStreakDays >= 3 ? "Burnout Risk" : stressData.highStreakDays >= 2 ? "Monitor" : "Normal" },
                        { param: "Burnout Risk", value: stressData.burnoutRisk ? "YES" : "NO", color: stressData.burnoutRisk ? "#EF4444" : "#10B981", cat: stressData.burnoutRisk ? "Consult Doctor" : "Safe" },
                      ].map((row, i) => (
                        <View key={i} style={[styles.tableRow, i % 2 === 0 && { backgroundColor: "#FAFBFC" }]}>
                          <Text style={[styles.tdCell, { flex: 2 }]}>{row.param}</Text>
                          <Text style={[styles.tdCell, { flex: 1.5, fontFamily: "Inter_600SemiBold" }]}>{row.value}</Text>
                          <Text style={[styles.tdCell, { flex: 1, color: row.color, fontFamily: "Inter_600SemiBold", fontSize: 9 }]}>{row.cat}</Text>
                        </View>
                      ))}
                    </View>
                    <View style={{ backgroundColor: "#F5F0FF", borderRadius: 8, padding: 10, marginTop: 10 }}>
                      <Text style={{ fontFamily: "Inter_700Bold", fontSize: 9.5, color: "#7C3AED", marginBottom: 4 }}>🧠 STRESS INTERPRETATION</Text>
                      <Text style={{ fontFamily: "Inter_400Regular", fontSize: 9, color: "#4C1D95", lineHeight: 14 }}>
                        {(stressData.weekAvg || 0) < 26 ? "Your stress level is well-managed this week." :
                         (stressData.weekAvg || 0) < 51 ? "Moderate stress detected. Consider breathing exercises and adequate sleep." :
                         (stressData.weekAvg || 0) < 76 ? "Elevated stress this week. Prioritize rest and reduce workload where possible." :
                         "High stress levels detected. Please consult your healthcare provider."}
                      </Text>
                    </View>
                  </View>
                </>
                ...
                <View style={{ flexDirection: "row", gap: 8, alignItems: "flex-start" }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: "#78350F", lineHeight: 16 }}>
                      {overall >= 70
                        ? `Your health activity this ${reportType === "weekly" ? "week" : "month"} is ${getActiveLabel(overall).toLowerCase()}. Keep up your habits — consistency is the key to long-term health.`
                        : `Your ${reportType === "weekly" ? "weekly" : "monthly"} score is ${overall}%. There is room to improve — focus on ${(card?.activePercent?.waterPct ?? 0) < 50 ? "drinking more water 💧" : (card?.activePercent?.foodPct ?? 0) < 50 ? "logging your meals 🍛" : "exercising daily 🏃"}. Small steps make a big difference.`
                      }
                    </Text>
                  </View>
                </View>
              ...
              <View style={{ padding: 12, paddingTop: 8 }}>
                <Text style={{ fontSize: 7.5, color: "#9CA3AF", fontFamily: "Inter_400Regular", lineHeight: 12, textAlign: "center" }}>
                  This report is auto-generated by the Aorane Health platform and is for personal health tracking purposes only. It does not constitute medical advice. Please consult a qualified healthcare professional for any medical diagnosis or treatment.
                </Text>
              </View>
