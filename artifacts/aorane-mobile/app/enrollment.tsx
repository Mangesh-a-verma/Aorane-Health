...
export default function EnrollmentScreen() {
  ...
  const textColor = isDark ? "#F0F8FF" : "#1a1a2e";
  const subText = isDark ? "rgba(255,255,255,0.5)" : "rgba(10,22,40,0.5)";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let result: { planUpgraded: string; org: { name: string } } | null = null;
      try {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: bg }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <LinearGradient colors={isDark ? ["#010814", "#020C1E"] : ["#F0F9FF", "#FFF8F3"]} style={StyleSheet.absoluteFill} />
      <View style={{ paddingTop: insets.top + 8, paddingHorizontal: 16, flex: 1 }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 32 }}>
          <TouchableOpacity onPress={() => router.back()} style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,119,182,0.08)", alignItems: "center", justifyContent: "center" }}>
        ...
        {/* Code Input */}
        <View style={{ backgroundColor: cardBg, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,119,182,0.12)", marginBottom: 16 }}>
          ...
          <TextInput
            ...
            returnKeyType="done"
          />
          <View style={{ height: 2, backgroundColor: isDark ? "rgba(59,130,246,0.3)" : "rgba(0,119,182,0.2)", borderRadius: 2 }} />
        </View>
