...
export default function BloodEmergencyScreen() {
  ...
  const handleToggleAvailability = useCallback(async (available: boolean) => {
    ...
    try {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch((e) => logSilentError('haptics', e));
      Alert.alert(
  ...
  const searchDonors = async () => {
    if (!searchCity.trim()) { Alert.alert("Required", "Enter a city name or use the Nearby button"); return; }
    setSearching(true);
  ...
  const registerDonor = async () => {
    if (!donorCity.trim() || !donorState.trim()) { Alert.alert("Required", "Please enter City and State"); return; }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <LinearGradient colors={["#DC2626", "#B91C1C"]} style={{ paddingTop: (insets.top) + 12, paddingHorizontal: 16, paddingBottom: 0 }}>
        ...
        <View style={{ flexDirection: "row", gap: 6, paddingBottom: 12 }}>
          {([["requests", "📋 Requests"], ["find", "🔍 Find Donor"], ["donate", "❤️ Donate"]] as [string, string][]).map(([t, label]) => (
            <TouchableOpacity key={t} onPress={() => { setTab(t as typeof tab); Haptics.selectionAsync().catch((e) => logSilentError('haptics', e)); }}
        ...
        {tab === "requests" && (
          <View style={{ gap: 12 }}>
            ...
            <View style={{ flexDirection: "row", backgroundColor: "#F3F4F6", borderRadius: 12, padding: 3 }}>
              {([["all", "📋 All Requests"], ["mine", "🙋 My Requests"]] as const).map(([v, label]) => (
              ...
              <TouchableOpacity onPress={searchNearbyDonors} disabled={searching || gpsLoading} activeOpacity={0.85}>
                <LinearGradient colors={["#DC2626", "#B91C1C"]} style={styles.actionBtn}>
                  ...
                  <Text style={{ color: "#FFF", fontFamily: "Inter_700Bold", fontSize: 15 }}>
                    {searching && nearbySearch ? "Searching via GPS..." : "📍 Nearby Donors (50 km)"}
                  </Text>
                </LinearGradient>
              </TouchableOpacity>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {radiusExpanded && (
                    <Text style={{ color: C.muted, fontSize: 11, fontFamily: "Inter_400Regular", marginTop: 2 }}>
                      Only a few donors were found within 50 km, so the search was automatically expanded to {searchedRadius} km.
                    </Text>
                  )}
                </View>
              </View>
            ...
            {statusLoading ? (
              ...
              <Card>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <Text style={styles.cardTitle}>My Donor Status</Text>
      ...
      {/* ─── EMERGENCY REQUEST MODAL ────────────────────────────────────────── */}
      <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => { setShowModal(false); resetModal(); }}>
        <View style={{ flex: 1, backgroundColor: C.bg }}>
          ...
          {formStep === "form" ? (
            <ScrollView contentContainerStyle={{ padding: 18, gap: 4, paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <FieldLabel text="Units Required" required />
