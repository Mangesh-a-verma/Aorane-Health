 ...
 * Optimized for Direct Banner Ads & Fast Performance
 */
...
const PLACEHOLDER_GRADIENTS: [string, string][] = [
  ["#0077B6", "#00B896"], ["#0EA5E9", "#38BDF8"], ["#059669", "#10B981"],
  ["#0369A1", "#0284C7"], ["#00B896", "#34D399"],
];

const PLACEHOLDER_EMOJIS = ["🏆", "👨‍👩‍👧‍👦", "🥗", "💊", "❤️"];
const PLACEHOLDER_SUBS = ["Upgrade now →", "Add family members →", "Try AI scan today →", "Set reminders →", "See your score →"];
...
function AdSlide({ ad, index, onImpression }: { ad: Ad; index: number; onImpression: (id: string) => void }) {
  ...
  // Safe Deep Linking Fix
  const handlePress = async () => {
    const targetUrl = ad.linkUrl;
    ...
    try {
      ...
      const finalUrl = res.linkUrl || targetUrl;
      if (finalUrl) {
        const canOpen = await Linking.canOpenURL(finalUrl);
        if (canOpen) await Linking.openURL(finalUrl);
      }
    } catch {
      if (targetUrl) {
        const canOpen = await Linking.canOpenURL(targetUrl);
...
export function AdsSlider() {
  ...
  const loadAds = useCallback(async () => {
    ...
    try {
      const res = await api.getActiveAds("dashboard") as { ads: Record<string, unknown>[]; adsEnabled?: boolean };
      if (res.adsEnabled === false) { setAds([]); setLoading(false); return; }
      
      const fetched = (res.ads as Ad[]) || [];
      // 1-Ad Bug Fixed: length > 0 handle karega
  ...
  return (
    <View style={st.wrapper}>
      <FlatList
        ...
        onScrollBeginDrag={handleScrollBeginDrag} // UX Update
