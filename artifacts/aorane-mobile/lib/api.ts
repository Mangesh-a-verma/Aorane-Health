...
async function clearAuthAndNotify() {
  ...
  await AsyncStorage.multiRemove(NON_SENSITIVE_AUTH_KEYS).catch(() => {});
  _onUnauthorized?.();
}
...
export const api = {
  ...
  createSubscription: (plan: string, promoCode?: string) =>
    request<{ subscriptionId: string; razorpaySubscriptionId?: string; razorpayKeyId?: string; plan: string; amount: number; discount: number; isTestMode: boolean; expiresAt?: string; message?: string; accessToken?: string; refreshToken?: string }>("POST", "/payment/subscription/create", { plan, promoCode }),
